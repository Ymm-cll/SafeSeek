...
class AblatedLinear(nn.Module):
    ...
    def get_mask(self, temperature=1.0, hard=True, add_noise=False):
        if self.training and hard and add_noise:
            eps = 1e-10
            u = torch.rand_like(self.logits)
            gumbel_noise = -torch.log(-torch.log(u + eps) + eps)
            logits_to_use = self.logits + gumbel_noise
        else:
    ...
    def load_masks(model, path, map_location="cpu"):
        ...
        count = 0
        for name, module in model.named_modules():
            if isinstance(module, AblatedLinear) and name in loaded_masks:
                module.logits.data.copy_(loaded_masks[name].to(module.logits.device))
                count += 1
        print(f"[AblatedLinear] Loaded {count} masks from {path}")
    ...
    for name, module in model.named_modules():
        for target in layers_to_patch:
            if name.endswith(target):
                ...
                mask_params.append(new_module.logits)
    return mask_params
...
class CircuitTrainer(Trainer):
    ...
    def _format_mask_stats(self, stats):
        ...
        return [
            ...
            f"  Active neurons  : {active:7d}/{total:7d} ({100-sparsity:5.2f}%)",
            f"  Inactive neurons: {zeros:7d}/{total:7d} ({sparsity:5.2f}%)",
        ]
...
def train_circuits():
    ...
    if TEST_MASKS_PATH is not None and os.path.exists(TEST_MASKS_PATH):
        print(f"Loading test masks from: {TEST_MASKS_PATH}")
        AblatedLinear.load_masks(model, TEST_MASKS_PATH)
        
        trainer_test = CircuitTrainer(
            model=model,
            mask_params=mask_params,
            args=TrainingArguments(output_dir=OUTPUT_DIR, per_device_eval_batch_size=8),
        ...
        generate_and_save_results(model, tokenizer, val_data, 8, "post_mask_accept.json", use_inverse_mask=False)
        generate_and_save_results(model, tokenizer, val_data, 8, "post_mask_refuse.json", use_inverse_mask=True)
        return
    ...
    generate_and_save_results(model, tokenizer, val_data, 8, f"{OUTPUT_DIR}/post_mask_accept.json", use_inverse_mask=False)
    generate_and_save_results(model, tokenizer, val_data, 8, f"{OUTPUT_DIR}/post_mask_refuse.json", use_inverse_mask=True)
