...
class CircuitTrainer(Trainer):
    def __init__(
        ...
        circuit_backdoor_weight=0.0,
        circuit_clean_weight=0.0,
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        circuit_sparsity_loss = sum([torch.sigmoid(p).mean() for p in self.circuit_mask_params])

        def compute_overlap_loss(m):
            total_ratio = 0.0
            count = 0
            for mod in m.modules():
                if isinstance(mod, MaskedAttentionWrapper):
    ...
    def _format_mask_stats(self, stats):
        def _line(label, total, zeros):
            ...
            active = total - zeros
            return f"  {label}: active={active:4d}/{total:4d} ({100-sparsity:5.2f}%) | inactive={zeros:4d} ({sparsity:5.2f}%)"
...
def train_circuits():
    ...
    print(f"Number of mask parameters to optimize: Main={len(main_mask_params)}, Circuit={len(circuit_mask_params)}")
